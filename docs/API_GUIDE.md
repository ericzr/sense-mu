...
本文是给后端开发同事的接口地图，强调调用方、授权方式和业务顺序。首次接入请先阅读 `docs/DEVELOPER_HANDOFF.md` 的正式地址、架构边界和 P0/P1 优先级，并以 `docs/DOMAIN_MODEL.md` 统一理解项目、数据版本、训练任务、模型版本、服务和商品的边界。字段级请求/响应以运行中的 OpenAPI 为唯一准确信源：
...
生产配置还必须使用 HTTPS 的公开地址、非本机依赖和不可变训练镜像摘要。Webhook 地址创建时拒绝本地/内部域名；Worker 投递前会再次解析并拒绝解析到 loopback、私有或保留网络的目标。生产网络仍应在出口层配置拒绝私网的兜底规则。
...
模型版本响应统一返回 `task_type`、冻结 `dataset_version_id`/版本号、`class_map`、训练框架、执行器、recipe、基础模型、产物文件名、大小和 SHA-256。`artifact_uri` 是内部事实字段，不进入浏览器响应；下载必须通过上面的工作区授权接口，服务端会先按登记的大小和 SHA-256 核对对象完整性，再在生产环境返回短时签名地址，或在本地开发由 API 代理并复核返回内容。格式转换不属于当前接口合同。

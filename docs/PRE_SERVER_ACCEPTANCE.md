...
| 领域 | 证据 | 结果 |
|---|---|---|
| API 与状态机 | API、Worker、网关、Runtime 单元/集成测试 | 101 项通过 |
...
| 前端 | 渲染、类型、Lint、OpenAPI 类型、托管预览 E2E | 渲染 18 项、托管预览 6 项通过 |
...
- 空 SQLite 数据库已从初始版本升级到 `20260924_0038`，包含训练资源快照和在线服务托管方式迁移。
- 真实本地 API 浏览器 E2E 10 项全部通过，覆盖会话失效、资源恢复、数据标注、训练入队、独立验收、安全发布、项目归档与暂停/继续失败恢复。
- API/Worker/网关/Runtime 测试 101 项、Ruff、Python 编译、OpenAPI 漂移检查均通过。
- 当前开发机 Docker daemon 未运行，因此训练容器、GPU 驱动、镜像 digest、队列积压和容器级恢复尚未取得真实环境证据；不能用本地 CPU 或演示站结果替代。
...
- Core API 的 `/health/ready` 必须同时通过数据库、对象存储和 Redis 探测；任一依赖不可用时返回 `503`，部署平台不得继续导入流量。

...
- 正式上线执行手册：[`docs/PRODUCTION_GO_LIVE_RUNBOOK.md`](docs/PRODUCTION_GO_LIVE_RUNBOOK.md)
...
完整基础设施使用 Node.js 24.x（仓库固定为 `24.19.0`）与 Docker。首次进入仓库先执行 `nvm use`；`npm install`、开发、构建、检查都会拒绝其他主版本，避免 Vinext 在 Node 20 上出现不明确的内部错误：

...
真实写入套件覆盖工作区、项目、数据集、素材上传、数据划分、类别、人工标注、审核完成、版本冻结、训练入队、独立验收、服务发布和 API 密钥轮换。训练与验收的 Worker 结果通过受控内部接口登记，以便在没有 GPU Worker 的隔离测试环境验证真实控制面；发布与密钥行为仍由浏览器操作页面完成。首次密钥在创建或轮换响应中仅显示一次，刷新后的列表只返回脱敏前缀。上传、冻结版本、训练创建、发布与一次性密钥展示必须连接本地 Core API 与隔离测试数据库后再做写入验收，不能由只读演示站替代。
